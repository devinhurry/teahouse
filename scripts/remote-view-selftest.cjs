...
async function run() {
  ...
  desktopCapturer.getSources = async () => [{ id: 'screen:test:0', name: '本地合成测试屏幕', display_id: 'test',
    thumbnail: (await source.webContents.capturePage()).resize({ width: 240 }), appIcon: null }]
  ...
  let sampled = 0, displayed = 0, autoAccept = true, lastSize = null, slowSample = false
  ...
  display: async (_id, _seq, bytes, width, height) => {
    ...
    displayed++; lastSize = { width, height }
  } })
  ...
  await until(() => !viewer.isDestroyed() && viewer.webContents.executeJavaScript('!!document.querySelector("canvas") && getComputedStyle(document.querySelector("canvas")).display !== "none"'), '第一帧解码')
