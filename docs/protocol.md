...
- 控制报文新增 `type:"screen"`。`request/accept/reject` 复用 `Messenger.sendReliable` 的 ACK 与 TCP 控制帧回退，**不入离线队列、不存聊天记录**；`end` 尽力单发并立即本地清理，不等待 ACK 才停止。屏幕报文另用内存去重（按节点+消息 ID，最多 256 条、120 秒），不触碰持久 dedup；取消时中止 request/accept 的重试与 TCP 等待，取消本身不将对端标离线。
