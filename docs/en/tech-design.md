...
| Field | Value |
|---|---|
| Current design | v1.81; v0.58.0 implements remote desktop viewing (#310); physical-platform acceptance pending |
...
5. Main validates bounded bytes and JPEG metadata before IPC. The viewer fully decodes a local Blob, checks dimensions, draws Canvas, revokes the URL and acknowledges consumption. No animation-frame dependency when minimized. Enlargement cannot recover lost source detail.
...
Only the viewer can change mode. Changes reset rate counters and affect subsequent requests without reconnecting, recapturing or changing image quality. Real throughput may be below the selected target. Minimized viewers temporarily request 3 fps, then restore the selected mode. Media capture itself is capped at 10 fps; reduced request rate does not promise proportional OS capture-cost savings. Preserve Win7/Linux software rendering.
