...
> 当前更新：2026-09-18，**v0.60.1 联系人资料同步修复（#316）**。`app-state` 统一资料/能力版本递增与持久化，`PeerRegistry` 对同版本完整资料的实际变化通知界面与落库；下一决议 **#317**。本轮本地修复，尚未发布。
> 本轮验证：124 个测试文件 / 823 测试、Electron ABI 数据库、类型、构建、隔离 smoke、版本一致性全部通过。真实 Electron 22 回环验证公司/部门/团队独立修改及清空、通讯录分组与已打开资料页、同版本公司变化、重复资料不推送、本地备注保留、联系人落库及设置输入失焦保存；启动能力版本与配置一致，保存公司后继续递增。仅本机回环验收，Win7/UOS/麒麟目标机仍待复测。
