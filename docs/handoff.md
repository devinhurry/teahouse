...
> 最后更新：2026-09-17（**v0.59.1 屏幕协助卡片按发起方左右排列（#313）；目标机权限和性能待验收**）。仍遵守 Electron 22.3.27 / Node16 / Chrome108 / 纯内网红线；当前状态以 `git log` 与各文档变更记录为准。
> 本轮入口：`MessageRow.vue` → `ScreenHistoryCard.vue`，复用持久化的 `isMine` 与普通消息行布局；无迁移/协议改动。下一决议 **#314**。
