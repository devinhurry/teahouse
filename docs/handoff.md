...
> 最后更新：2026-09-17（**v0.59.0 屏幕协助贴边条、聊天记录及发起确认（#312）；目标机权限和性能待验收**）。仍遵守 Electron 22.3.27 / Node16 / Chrome108 / 纯内网红线；当前状态以 `git log` 与各文档变更记录为准。
> 本轮入口：`ScreenRequestDialog.vue` → `RemoteViewService` → `ChatService.recordScreen` → 本地系统卡片 `ScreenHistoryCard.vue`；新记录用现有 SQLite 元数据，无迁移/协议改动。下一决议 **#313**。
